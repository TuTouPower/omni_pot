...
| `npm run dist` | 构建 + 打包（NSIS 安装版 + 便携版；Windows 上成功后自动静默重装并启动安装版；若既有 release 产物被 Omni Pot 占用，会在打包前自动关闭） |
...
仓库不提交大体积二进制；首次本地开发或打包前需要生成Chinese Dictionary DB（`npm run dist` 已经在 `scripts/run_dist.mjs` 里自动执行，纯本地 `dev`/`build` 路径可按需手动跑）：

```bash
npm install
npm run build:chinese-dictionary   # 依赖项目内 github_repo/chinese-dictionary 或 WSL 上游 \\wsl.localhost\Ubuntu-22.04\home\karon\karson_ubuntu\github_repo\chinese-dictionary
```
