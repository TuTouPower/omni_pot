...
应用的用户数据存放在 Electron `app.getPath('userData')` 目录下，由 `app.name` 决定：

| 环境 | `app.name` 来源 | Windows 路径 |
|---|---|---|
| 开发 (`npm run dev`) | `package.json` → `omni_pot` | `%APPDATA%\omni_pot\` |
| 打包产物 | `electron/main.ts` → `Omni Pot` | `%APPDATA%\Omni Pot\` |
