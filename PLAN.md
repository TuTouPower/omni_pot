...
- [ ] **快捷键展示格式统一**：托盘、快捷键页、欢迎页统一显示 `Ctrl + Alt + T` / `Cmd + Opt + T`，不用 `Control`，也不用无空格格式。
- [ ] **各窗口置顶状态独立**：翻译、词典、文字识别、截图翻译分别维护 pin 状态；词典 / 识别 / 截图翻译不复用 `translate_always_on_top`。
- [ ] **文字识别设置按 spec 最小集合实现**：默认识别语言为“自动检测”；自动去除换行、自动复制结果、失焦时关闭、识别后隐藏窗口默认开启；暂不实现设计稿扩展字段。
- [ ] **本地 API 端口问号位置**：通用页“本地 API 端口”的 `?` 移到 label 旁，使用小圆圈问号样式。
- [ ] **通用页托盘点击行为**：补 `tray_click_event` UI 入口。
- [ ] **结果卡片朗读 busy 态**：翻译结果卡片朗读请求中显示 loading dots，与源词区三态一致。
- [ ] **复制图片 tooltip**：文字识别 / 截图翻译窗口复制图片按钮 tooltip 固定为“复制图片”。

### P2

- [ ] **主卡片 / 主要按钮圆角统一 10px**：pill、圆形 icon button、颜色圆点等小元素保留 pill / 圆形语义。
- [ ] **CSS 初始主色同步默认天蓝**：配置加载前的 `--brand-primary` 与默认主色 `#5a9bbf` 保持一致。
- [ ] **翻译窗口固定按钮锁芯反色**：固定按钮激活后内部线条按主题反色。
- [ ] **快捷键解绑按钮视觉**：若仍有 Check 图标，移除并保留纯文字“解绑”。
- [ ] **托盘词典 icon 对齐 demo**：词典入口 icon 与 demo 保持一致。
