...
3. 结果区始终渲染。每个引擎对应一张卡片,初始为"折叠态"——只显示卡片头部一行(引擎名 + 朗读/复制/折叠按钮);当该引擎的翻译结果返回后,卡片自动展开 body,窗口高度立即扩张(N 个引擎产生 N 次扩张)。**"没结果时折叠"指的是卡片本身处于折叠态,不是结果区整体不渲染。**
4. 当内容总高度 > 窗口最大高度时:窗口锁定在最大高度,结果区内部出现滚动条;**滚动时输入区固定在窗口顶部不动,只有结果卡片滑动**。外层布局必须用 flex 显式约束高度,使滚动仅发生在 `ResultsScroll`,而非整个文档。
...
- **结果区始终渲染**(不做整体 `display:none`)。当用户尚未触发翻译,或翻译已触发但还没有任何引擎返回时,卡片列表仍存在,只是每张卡片处于**折叠态**——只展示 header 一行。
- 引擎返回结果后,对应 `<TargetArea>` 的 body 展开,显示翻译文本;窗口高度通过 IPC 重算扩张。
- 用户点击卡片 header 的折叠按钮 → body 隐藏,header 仍占一行。
- 多引擎卡片纵向堆叠在 `ResultsScroll` 内。
- 当未配置任何引擎时,结果区为空(没有任何卡片),窗口仅保留输入区高度。

### 滚动行为

- **外层布局显式约束高度**:`#app` 用 `display:flex; flex-direction:column; height:100vh`(实际高度由主进程通过 `setBounds` 控制窗口尺寸,渲染端用 100vh 填满)。`LanguageArea`/`SourceArea`/`ActionBar` 高度自适应,`ResultsScroll` 用 `flex:1; min-height:0; overflow-y:auto` 占据剩余空间并独自滚动。
- 这样在窗口被锁定在 `max_h` 时,溢出仅出现在 `ResultsScroll` 内,文档本身不滚动。
- `SourceArea` 永远在窗口顶部不参与滚动,滚动条只出现在 `ResultsScroll` 内。
- **明确不依赖**全局 `window:setContentHeight` 这类自适应机制——翻译窗口走专属 IPC(见 §6)。
...
- 不改造其他窗口(截图、词典、设置等)的尺寸策略;通用 `window:setContentHeight` 保留原样,翻译窗口的规则不外溢到其他窗口。
- 不增加用户可配置项(`MAX_HEIGHT_RATIO` 等常量内置)。
- 不删除/合并/重写已有的通用窗口尺寸 IPC——翻译窗口走独立新通道,二者并存互不干扰。
