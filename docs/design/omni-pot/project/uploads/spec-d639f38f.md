...
| 数据库 | better-sqlite3（翻译历史、中文词典；native 模块由 `electron-builder install-app-deps` rebuild，打包时仅将 `*.node` 解包） |
...
- **中文词典卡片**隐藏词性标签（POS tag）和朗读按钮；英文词典卡片保留词性标签，有 `audioUrl` 时播放原声，无 `audioUrl` 时不显示朗读按钮；词典结果卡片不设整段朗读按钮
...
`cc-cedict@default`、`cambridge_dict@default`、`system_tts@default`、
`system@default`、`qrcode@default`。
...
| 19 | CC-CEDICT | `cc-cedict` | 离线（CC-CEDICT SQLite），输出词典结果 |
...
| `dictionary_service_list` | string[] | `['cc-cedict@default']` | 中文词典实例顺序 |
...
`tray:popup-ready`、`chinese-dict:state-changed`。
...
- 词典窗口顶部为可编辑源词卡片（contentEditable div，Enter 重查），下方为发音卡片，再下方为按词性分组的释义（每个词性作为小标题，下方编号义项从 01 开始）；按检测语言路由到对应词典列表（英文→英文词典列表、中文→中文词典列表）；不再显示词形变化卡片与来源 chips
...
- 服务设置 5 个 Tab：翻译 / 中文词典 / 英文词典 / 文字识别 / 语音朗读；编辑、删除使用带文字的完整按钮；无插件相关入口
...
剪贴板监听、翻译历史记录（SQLite）、中文词典、设置页完整实现。
