...
`windows/other.jsx` `TrayMenu` 包含以下菜单项：输入翻译、文字识别、截图翻译、剪贴板监听（勾选）、自动复制（子菜单）、设置、检查更新、查看日志、重启、退出。

当前 `electron/tray/index.ts` 实际实现的菜单项为：输入翻译、文字识别、截图翻译、剪贴板监听（checkbox）、设置、检查更新、查看日志、重启、退出。

差异：

- example 用”截图翻译”，代码用”截图翻译”——名称不同。
