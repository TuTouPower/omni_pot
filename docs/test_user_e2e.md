...
> E2E 基础设施已部分落地；剩余项跟踪进度见 `TASKS.md`。
...
- 第三排操作区交互（文字识别模式按钮顺序：`复制图片 → 识别引擎下拉 → 自动检测 | 翻译 → 去除换行 → 去除空格 → 复制 → 导出`；截图翻译模式按钮顺序：`复制图片 → 识别引擎下拉 → 自动检测 → 转换符号 → 简体中文 | 去除换行 → 去除空格 → 复制 → 导出`）：
  ...
  - **去除换行** → 结果换行被去除（连字符断行合并 + 多空白合一）
  - **去除空格** → 结果空格被去除，换行符保留不变
  - **复制** → 剪贴板为识别文本
...
- 搜索 + 服务筛选 + 时间筛选组合过滤
- 工具栏 UI 验证：所有控件同一行（y 坐标差 < 50px）、"启用"标签在首位、搜索框含搜索图标、时间筛选选项为"全部时间/今天/本周/本月"、禁用开关后其余控件置灰
...
- “稍后提醒”关闭窗口；“立即更新”从本地 HTTP 资产下载，显示进度到 100%，并走 E2E 安装成功分支
- 生产环境只允许本仓库 GitHub release asset URL 与 GitHub 可信重定向；本地 HTTP 资产只在 `OMNI_POT_E2E=1` 测试环境中允许
...
- 更新检查：`/e2e/mock-update` 注入假“有新版本”数据，避免依赖真实 GitHub release ——
  这只是数据桩，用户操作（点更新器按钮）仍是真实的；下载 IPC 只传 asset name，URL 由主进程绑定的 release metadata 决定；生产环境限制为本仓库 GitHub release asset 与 GitHub 可信重定向，并要求 sha256 digest 校验；测试环境才允许 localhost HTTP 资产。
