...
1. **覆盖完整**：覆盖 `docs/design/example/` 设计稿出现的每个 UI 元素与交互，
   以及代码已实现的每个窗口与功能。设计稿与 spec 的差异处理见 `docs/design/example_todo.md`。
...
- 内容区顶栏：页面名 + 路由 + 关闭按钮
- **通用页**：主题/界面语言/字体/字号/透明/开发者模式/API 端口/开机自启/检查更新
