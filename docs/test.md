...
| 层级 | 目录 | 框架 | 职责 |
|---|---|---|---|
| 单元测试 | `tests/unit/`、`tests/detect/`、`tests/chinese_dict/` | Vitest | 服务接口、工具函数、状态管理逻辑、检测/词典数据脚本 |
...
```bash
# 单元 + 集成测试
npx vitest run tests/unit tests/integration tests/detect tests/chinese_dict
...
| 单元 + 集成 | `npx vitest run tests/unit tests/integration tests/detect tests/chinese_dict` | 模块正确性、配置读写、服务逻辑、检测/词典数据脚本 |
