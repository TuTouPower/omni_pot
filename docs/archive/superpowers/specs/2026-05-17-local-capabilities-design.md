...
打包时 `package.json` `build.extraResources` 配置直接包含该路径：
...
在 `package.json` `build.extraResources` 数组中添加：
...
- `detect.local` bridge 类型声明在 `shared/types/ipc.ts` 的 `ElectronAPI` 接口中（与现有 `dict`、`text` 等 bridge 一致）
