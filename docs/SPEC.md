...
17. [配置系统](#17-配置系统)
18. [IPC 通道](#18-ipc-通道)
19. [HTTP API 服务器](#19-http-api-服务器)
20. [系统托盘](#20-系统托盘)
21. [全局快捷键](#21-全局快捷键)
22. [剪贴板监听](#22-剪贴板监听)
23. [跨平台选中文本提取](#23-跨平台选中文本提取)
24. [翻译历史记录](#24-翻译历史记录)
25. [备份与恢复](#25-备份与恢复)
26. [自动更新](#26-自动更新)
27. [国际化](#27-国际化)
...
29. [测试策略](#29-测试策略)
30. [状态细节](#30-状态细节)
31. [UI 验收标准](#31-ui-验收标准)
32. [分阶段交付计划](#32-分阶段交付计划)
...
- **词典** — 选中字词查询词典（英文走英文词典、中文走 Chinese Dictionary）
- **文字识别** — 区域截图 → 文字识别
- **截图翻译** — 截图 → 识别 → 自动翻译
...
| 数据库 | better-sqlite3（翻译历史、Chinese Dictionary；native 模块由 `electron-builder install-app-deps` rebuild，打包时仅将 `*.node` 解包） |
...
| 翻译 | `translate` | 宽度记忆值或默认宽度；高度自适应内容 | 语言转换区自然宽度 + 少量留白 / 内容自然最小高度 | 未固定时失焦自动关闭、可固定、可置顶、frameless；宽度只能由用户调整并按配置记住，不允许内容或程序自动拉宽；高度随内容自适应 |
| 欢迎 | `welcome` | 520×560 | 520×560 | 首次启动且 `welcome_dismissed=false` 时打开；不可调整大小；不显示置顶 / 固定按钮；不是翻译窗口空状态 |
...
`src/main/windows/manager.ts` 的 `WindowManager`：
...
5. 注册全部 IPC handlers（config / window / hotkey / text / ocr / history / backup / chinese_dict）
6. 初始化Chinese Dictionary状态机
7. 创建系统托盘
8. 从配置注册全局快捷键
9. 启动 HTTP 服务器（端口占用时重试 5 次）
10. 如 `clipboard_monitor=true` 启动剪贴板监听
11. 创建 daemon 窗口（隐藏）
12. 按 `welcome_dismissed` 决定是否创建欢迎窗口：未跳过时创建 `welcome`，已跳过时不自动创建翻译窗口
13. 首次运行标记完成
14. 检查更新

CSP 保持默认同源限制，但 `connect-src` 允许 `https:` 外部连接与本机 HTTP 端点，以支持翻译、OCR、TTS、Ollama、DeepLX、Anki 等服务 API；`media-src` 允许 `blob:` 与 `https:`，以支持本地合成音频和 Cambridge 等词典原声音频播放；`worker-src` 允许 `blob:`，且 `script-src` 允许 WebAssembly 执行，以支持 Tesseract.js OCR worker/core。
...
翻译、词典、文字识别、截图翻译窗口顶部栏均有置顶按钮和固定按钮，从左到右：**置顶按钮、固定按钮**、显示名 `Omni Pot`、当前模式标签。

欢迎窗口、设置窗口与更新器窗口**不显示**置顶 / 固定按钮；欢迎窗口右上角只保留关闭按钮，设置窗口与更新器窗口右上角使用普通的最小化 / 最大化 / 关闭三件套。
...
- **宽度下限**：主进程以 280px 作为硬保底；语言转换区渲染后，按其自然宽度（完整展示“自动检测 + 转换符号 + 简体中文”）加少量左右留白实时更新实际下限
- 用户不能把翻译窗口缩到低于该宽度下限
- **宽度无上限要求**，但宽度只能由用户手动调整；程序、输入内容、翻译结果或内部自适应逻辑都不得自动拉宽窗口
- 开启窗口尺寸记忆时，只记住用户调整后的宽度；翻译窗口高度始终由内容重新计算，不记忆用户调整后的高度
- **高度需要自适应内容**；源文本、结果卡等内容变化可以驱动窗口高度变化，但不能影响窗口宽度
- 高度由主进程按 `clamp(内容自然高度, 初始最小高度, floor(当前显示器工作区高度 × 0.75))` 控制；用户只能拖拽调整宽度，不能把高度拖离当前内容高度
- 源文本超过 8 行时由输入框内部滚动消化；翻译结果超过 75vh 上限时只滚动结果区，顶部栏、语言区和源文本区保持固定
...
- **去除空格按钮**：仅移除空格字符（`/ +/g → ''`），不影响换行符和其他空白
- **去除换行按钮**：用 `/-\s+/g → ''` 和 `/\s+/g → ' '` 规范化（连字符断行合并 + 多空白合一）
...
每个已启用的翻译服务实例渲染一张结果卡片，卡片可拖拽排序。未触发翻译且没有历史结果时不渲染结果卡片；用户清空源文本时同步清空结果区，让窗口回到初始高度。
...
- **目标语言锁定**：目标语言一旦确立（通过 `auto` 检测回退或用户手动选择），后续源语言的任何变更都不改变目标语言。例如源=`auto` 检测到中文 → 目标回退为 `en`，用户随后手动将源改为 `zh_cn`，目标保持 `en` 不变。此规则同时适用于翻译窗口（§7）和截图翻译窗口（§8）。
...
- 源文本为空时仍显示正常 SourceArea 与 LanguageArea
- 未触发翻译且没有历史结果时不渲染结果卡片
- 欢迎页不属于翻译窗口空状态，翻译窗口内不得渲染欢迎内容

### 5.6 欢迎窗口

**文件**: `src/windows/welcome/index.tsx`

- 独立 `welcome` BrowserWindow，默认 520×560，不可调整大小
- 顶部栏显示 `Omni Pot` 与“欢迎”，不得显示 `titlebar-mode=翻译`，不得显示置顶 / 固定按钮
- `welcome_dismissed=false` 时启动显示欢迎窗口；热键、托盘和 API 路径不得打开或闪现欢迎窗口
- 欢迎页中的快捷键提示按当前配置展示；未设置时显示未设置占位
- 快捷键提示必须按平台展示修饰键：Windows / Linux 显示 `Ctrl`，macOS 显示 `Cmd`；不得把 `CommandOrControl` 原样展示给用户
- 点击“翻译 / 词典 / 文字识别 / 截图翻译 / 设置快捷键”后写入 `welcome_dismissed=true`，打开对应窗口或动作，然后关闭欢迎窗口
- 点击“跳过”后写入 `welcome_dismissed=true` 并关闭欢迎窗口
...
- **英文输入** → 调用英文词典列表（默认 `cambridge_dict`）
- **中文输入** → 调用 Chinese Dictionary 列表（默认 `chinese_dictionary@default`, `ecdict@default`）
...
- **Chinese Dictionary卡片**隐藏词性标签（POS tag）和朗读按钮；英文词典卡片保留词性标签，有 `audioUrl` 时播放原声，无 `audioUrl` 时不显示朗读按钮；词典结果卡片不设整段朗读按钮
...
2. **操作栏**（下方）：
   ...
   - 右侧两个 icon 按钮（从左到右）：复制单词（Copy）、查询（Type，品牌色）。

源词卡片下方的发音信息展示在各词典结果卡片内部，随各自服务的结果一同渲染，不设独立发音卡片。释义区域按词性分组，每个词性作为小标题（chip 样式），下方为该词性对应的编号义项（从 01 重新编号）。

### 6.3 数据流

```
按字典快捷键
  → src/main/hotkey/index.ts: triggerSelectionDictionary()
    → readSelectedText()
    → focusOrCreate(DICT)
...
Chinese Dictionary查询：

```
renderer: cc_cedict.translate(word)
  → window.electronAPI.chinese_dict.lookup(word)
    → IPC → src/main/ipc/chinese_dictionary_handlers.ts
      → 单字 → characters 表查询
      → 词语/成语 → words → idioms 表查询
      → FTS5 前缀搜索兜底
      → better-sqlite3 查询 chinese_dictionary.db → DictResult | null
```

数据源：mapull/chinese-dictionary JSON → `scripts/build_chinese_dictionary.ts` 构建 SQLite（320K 词、16K 字、50K 成语）。
...
- 截图翻译窗口中目标语言遵循 §7 的目标语言锁定规则：一旦确立后，识别语言变更不改变目标语言
...
- **外观卡片**：主题（`app_theme`：跟随系统 / 浅色 / 深色）使用 3 按钮分段控件，不使用下拉框；主色调（`app_primary_color`，5 个圆形选色按钮）、透明背景（`transparent`）、托盘点击行为（`tray_click_event`）
...
- **自动复制结果**（`recognize_auto_copy`）：开关，默认开启
...
录入行为：捕获组合键、Backspace 清除、绑定按钮在主进程注册、成功 / 失败提示；已被其他应用占用时在"状态细节"区域以红色提示，**不在快捷键设置页常驻展示**。快捷键展示必须按平台把 `CommandOrControl` 解析成用户可读修饰键，并使用简洁、带空格的缩写格式：Windows / Linux 显示 `Ctrl + Alt + T`，macOS 显示 `Cmd + Alt + T`。Wayland 限制属于故障排查或文档说明，不放快捷键页。
...
Tabs 切换**五类服务**：翻译 / 中文词典 / 英文词典 / 文字识别 / 语音朗读。页面整体布局、服务卡片、字段展示和交互必须按 `docs/design/omni-pot/` 设计稿对齐。

- 每类显示**服务实例**列表，支持启停、上移/下移与拖拽排序（顶部优先）
- 每个实例项：拖拽手柄、服务图标、实例名、启停开关、编辑按钮、上移按钮、下移按钮、删除按钮（编辑 / 删除使用**带文字的完整按钮**，不是小图标）；不显示实例 key 副文本，不显示 PLATFORM / OFFLINE 等标签 chip
...
- 备份目标（`backup_type`）：本地（默认）/ WebDAV（配置项保留；远端同步未实现）
- WebDAV：服务器地址、用户名、密码（当前仅保存配置，不执行远端备份 / 恢复）
- 操作：立即备份、从备份恢复
- 备份内容：设置（不含 API token、WebDAV 密码、服务密钥等凭据）、历史记录数据库
- 页面布局、文案、按钮和状态展示必须按 `docs/design/omni-pot/` 设计稿对齐
- 导出的备份 zip 必须可再次导入；恢复后非凭据配置和历史记录恢复正确，凭据需用户重新填写

### 9.10 设置页: 关于

Omni Pot logo + 版本号、简介、官网 / 文档 / 反馈 / 检查更新 / 支持作者链接、诊断信息（日志目录 / 设置目录 / 本机 API 地址）。"支持作者"按钮使用紫色字色（`#9b59b6`），点击用默认浏览器打开 `https://afdian.com/a/tutoupower`。日志目录和设置目录行各带一个复制路径按钮；设置目录显示实际 `userData` 路径而非静态文字。诊断卡片内以 `ConfigRow` 行展示"导出日志"操作，说明文案："最近 7 天的日志打包为 zip，可附在反馈中"。
...
renderer: useConfigStore
  ...
  → 写: config:set IPC
    → src/main/config/store.ts: setConfig(key, value)
...
**OCR 服务**（`shared/types/ocr_service.ts`）、**TTS 服务**（`shared/types/tts_service.ts`）遵循类似结构。
...
interface DictResult {
  type: 'dict'
  pronunciations: { region: string; phonetic: string; audioUrl?: string }[]
...
- `config.instanceName` 覆盖列表显示名；`config.enable=false` 时该实例保留在列表中但不参与翻译、词典、OCR、TTS 执行
- `translate_service_list` / `dictionary_service_list` / `recognize_service_list` /
  `tts_service_list` 储存的是**实例 key**
...
`chinese_dictionary@default`、`cambridge_dict@default`、`system_tts@default`、
`system@default`、`qrcode@default`、`ecdict@default`。
...
- 主路径：主进程 `src/main/detect/index.ts` 预加载 cld3-asm WASM，renderer 通过 `detect.local` IPC 调用本地检测。
- 兜底路径：WASM 加载失败、检测不可靠、未映射语言或 IPC 不可用时回退到 Unicode 正则。
- 中文长句（例如重复“我爱你”的文本）必须识别为简体中文，不得误判为日语。

检测结果与目标语言相同时，实际目标回退到 `translate_second_language`。回退后的目标语言被锁定，后续源语言变更不再改变目标语言（见 §7 目标语言锁定规则）。
...
- **机制**：`src/main/config/store.ts` 自实现 JSON 读写
- **前端访问**：`src/stores/config_store.ts`（Zustand）+ `config:get/set/getAll/getUserDir` IPC
...
| `app_font` | string | `'default'` | 主字体族（设置 UI 已隐藏，保留配置兼容） |
| `app_fallback_font` | string | `'default'` | 回退字体族（设置 UI 已隐藏，保留配置兼容） |
| `app_font_size` | number | `16` | 基础字号 px（设置 UI 已隐藏，保留配置兼容） |
| `transparent` | boolean | `false` | 窗口透明 |
| `check_update` | boolean | `true` | 启动时检查更新 |
| `server_port` | number | `20202` | HTTP API 端口，修改后需重启 |
| `server_api_token` | string | 首次启动随机生成 | HTTP API 认证 token；不通过 `/config` 公开 |
...
| `dict_window_width` | number | `350` |
| `dict_window_height` | number | `420` |
| `dict_always_on_top` | boolean | `false` |
| `dict_pinned` | boolean | `false` |
| `recognize_window_width` | number | `860` |
| `recognize_window_height` | number | `520` |
| `recognize_remember_window_size` | boolean | `true` |
| `recognize_always_on_top` | boolean | `false` |
| `recognize_pinned` | boolean | `false` |
...
| `dictionary_service_list` | string[] | `['chinese_dictionary@default', 'ecdict@default']` | Chinese Dictionary 实例顺序 |
| `english_dictionary_service_list` | string[] | `['cambridge_dict@default', 'ecdict@default']` | 英文词典实例顺序 |
| `recognize_service_list` | string[] | `['tesseract@default', 'system@default', 'qrcode@default']` | OCR 实例顺序 |
...
| 命名空间 | 方法 / 事件 |
|---|---|
| `window` | `close` / `minimize` / `maximize` / `setAlwaysOnTop` / `getLabel` / `setContentSize` / `setContentHeight` / `openConfig` / `onConfigNavigate` |
| `config` | `get` / `set` / `getAll` / `getUserDir` / `onChange`（`config:changed` 广播） |
| `hotkey` | `register` / `unregister` |
| `text` | `getSelection` / `writeClipboard` / `write_clipboard_image` / `onTranslateSelectionPending` / `onTranslateFromSelection` / `onInputTranslate` / `onTranslateFromApi` / `onTranslateFromClipboard` / `onDictLookup` / `onTranslateSelectionEmpty` / `onDictSelectionEmpty` |
| `ocr` | `captureScreenshot` / `open_recognize` / `sendToTranslate` / `system_recognize` / `onScreenshotShow` / `onRecognizeShow` |
| `history` | `add` / `list` / `count` / `update` / `delete` / `clear` / `service_keys` |
| `backup` | `create` / `list` / `listWithSize` / `restore` / `import` / `delete` / `getPath` |
| `dict` | `lookup` / `check` |
| `translate` | `reportContentHeight` / `reportMinWidth` |
| `shell` | `openExternal` |
| `log` | `getDir` / `export` / `write` |
| `chinese_dict` | `lookup` / `check` / `reload` / `onStateChanged` |
| `update` | `onRelease` / `checkLatest` / `downloadAndInstall` / `onDownloadProgress` |
| `tray` | `show` / `close` / `action` / `labels` / `clipboardMonitoring` / `popupReady` |
| `detect` | `local` |
| `ready` | renderer 加载完成时调用 `ready(label)`，发 `renderer:ready` |

主进程 → renderer 事件通道：`translate:selection-pending`、`translate:from-selection`、`translate:input-translate`、
`translate:from-api`、`translate:from-clipboard`、`dict:lookup`、`screenshot:show`、
`recognize:show`、`config:changed`、`update:release`、`update:download-progress`、
`tray:popup-ready`、`chinese-dictionary:state-changed`。

---

## 19. HTTP API 服务器

**文件**: `src/main/server/index.ts`

绑定 `127.0.0.1:{server_port}`（默认 20202），公共端点要求 `X-Omni-Pot-Api-Token` 等于 `server_api_token`。CORS 只允许 localhost / 127.0.0.1 来源，非法 Origin 在路由执行前返回 403。

| 方法 | 端点 | 说明 |
|---|---|---|
| POST | `/` | 翻译请求体文本 |
| POST | `/translate` | 翻译请求体文本 |
| POST | `/recognize` | 截图识别，请求体 `{ mode: "translate" }` 时执行截图翻译，否则执行文字识别 |
| POST | `/dict` | 词典查询，请求体 `{ text: string }`，聚焦词典窗口 |
| GET | `/config` | 返回公开配置 allowlist；不返回 `server_api_token` / WebDAV 凭据（E2E 模式下携带有效测试 token 时返回完整配置） |
| GET | `/history` | 翻译历史分页，查询参数 `page` / `page_size`，返回 `{ success, data, page, page_size, total }`；外部 API 默认截断 source/target 文本 |

E2E 测试专用端点（仅 `OMNI_POT_E2E` 环境变量启用）：

旧式端点：`POST /trigger-selection`、`POST /trigger-dict`、`POST /trigger-clipboard`、
`POST /trigger-clipboard-translate`、`GET /capture-clock`。

新式 `/e2e/` 命名空间：
`POST /e2e/open-window`、`POST /e2e/reset-config`、`POST /e2e/set-config`、
`POST /e2e/trigger-screenshot`、`POST /e2e/trigger-input-translate`、`POST /e2e/trigger-hotkey`、
`POST /e2e/hotkey-system-failures`、`POST /e2e/tray-action`、`POST /e2e/mock-update`、
`GET /e2e/clipboard`、`GET /e2e/clipboard-image`、`GET /e2e/window-state`、
`GET /e2e/primary-display`、`GET /e2e/tray-menu`。
...
**文件**: `src/main/tray/index.ts`

### 托盘菜单

中文 UI 下条目文案与显示，**每个功能项右侧显示当前绑定的快捷键**（使用 `Ctrl / Cmd + Alt / Shift` 缩写形式）。修饰键必须按平台展示：Windows / Linux 显示 `Ctrl`（如 `Ctrl + Alt + T`），macOS 显示 `Cmd`（如 `Cmd + Alt + T`）；不得把 `CommandOrControl` 原样展示给用户：
...
| 剪贴板监听 | 复选框 | 切换剪贴板监听 |
| — | 分隔线 | — |
| 反馈 | 按钮 | 用默认浏览器打开 `https://github.com/TuTouPower/omni_pot_release/issues` |
| 支持作者 | 按钮（紫色字色） | 用默认浏览器打开 `https://afdian.com/a/tutoupower` |
| 检查更新 | 按钮 | 立即检查更新 |
| 查看日志 | 按钮 | 打开日志目录 |
| — | 分隔线 | — |
| 重启 | 按钮 | 重启应用 |
...
（否则焦点切走会读错目标）。翻译入口允许选区读取与窗口创建并行：先发起读取，再立即创建 / 聚焦翻译窗口，读取完成后投递文本或进入空输入模式。快捷键显示必须按平台转换修饰键：Windows / Linux 显示 `Ctrl`，macOS 显示 `Cmd`，不得显示 `CommandOrControl`。

---

## 22. 剪贴板监听

**文件**: `src/main/clipboard/index.ts`
...
**文件**: `src/main/selection/`（详见 `docs/superpowers/specs/2026-05-08-selection-text-extraction-design.md`）
...
- **macOS**：当前实现暂返回 `unsupported-platform`；Accessibility API + `Cmd+C` 回退仍是缺口，未完成前不得宣称支持 macOS 划词
- **剪贴板回退**：备份全部格式 → 写 sentinel → 模拟复制 → 轮询变化 → `finally` 恢复
- 主入口不向业务抛错，保留 `reason` 供日志、权限引导、测试断言使用
- macOS 划词完成前返回 `unsupported-platform`；实现 Accessibility 权限检查后再返回 `permission-denied` 并引导用户
...
**文件**: `src/main/history/index.ts` + `src/main/ipc/history_handlers.ts`
...
**文件**: `src/main/backup/index.ts` + `src/main/ipc/backup_handlers.ts`

- 备份类型（`backup_type`）：本地（默认）/ WebDAV（配置项保留；远端同步未实现）
- `backup.create()` — 将 `config.json` + 历史记录数据库打包为本地 zip
- `backup.list()` — 列出本地可用备份
- `backup.restore(name)` — 读取本地 zip 解压覆盖
- 导出的 zip 必须可作为恢复输入重新导入；恢复流程必须验证配置和历史记录恢复正确
- 当前备份 zip 未加密且包含 `config.json`，可能包含 WebDAV 密码、provider API key、server token；加密 / 凭据迁移完成前不得把备份描述为安全同步方案

---

## 26. 自动更新

**文件**: `src/main/updater/index.ts`
...
- **空状态**：翻译窗口源文本为空且未被用户跳过时显示欢迎引导：标题"欢迎使用 Omni Pot"、副标题、**4 个快捷键提示卡（翻译 / 词典 / 文字识别 / 截图翻译，分别读取翻译单一快捷键 / `hotkey_selection_dictionary` / `hotkey_ocr_recognize` / `hotkey_ocr_translate`；未设置则显示"未设置"占位）**、"设置快捷键"按钮（打开设置窗口并跳到快捷键页）、"跳过"按钮（直接关闭当前窗口，并在本会话内不再显示欢迎页）。源文本一旦有内容欢迎区即消失，清空后若未跳过会再次出现。快捷键提示按平台显示 `Ctrl` 或 `Cmd`，不得展示 `CommandOrControl`。
...
- 翻译窗口宽度下限以 280px 为硬保底，并在语言转换区渲染后按其自然宽度加少量留白实时更新；宽度无上限，但只能由用户调整并记住，程序和内容不得自动拉宽
- 翻译窗口高度随内容自适应，上限为当前显示器工作区高度的 75%；用户只能拖宽度，不能拖离主进程锁定的内容高度；超出上限时只滚动结果区，源文本输入框最多展示约 8 行，超过后仅输入框内部滚动；等待翻译结果时结果卡片折叠并显示动效，结果返回后自动展开
...
- 翻译结果卡片右上角集中放置朗读、复制、折叠按钮；卡片不显示 `stream` 标签；等待结果时显示轻量翻译中动效
...
- 词典窗口顶部为可编辑源词卡片（contentEditable div，Enter 重查），下方为发音卡片，再下方为按词性分组的释义（每个词性作为小标题，下方编号义项从 01 开始）；按检测语言路由到对应词典列表（英文→英文词典列表、中文→Chinese Dictionary列表）；不再显示词形变化卡片与来源 chips
...
- 服务设置 5 个 Tab：翻译 / Chinese Dictionary / 英文词典 / 文字识别 / 语音朗读；编辑、删除使用带文字的完整按钮；无插件相关入口
- 设置 → 通用：界面语言归”应用”卡片；”本地 API 端口”标签旁有小圆圈问号按钮用于打开 API 文档；外观卡片含主题三按钮分段控件、主色调、透明背景、托盘点击行为；不提供代理功能或代理设置
- 系统托盘菜单中文化为 4 个功能项 + 设置 / 支持作者（紫色字色） / 更新 / 日志 / 重启 / 退出，每个功能项右侧显示快捷键（Windows / Linux 使用 `Ctrl`，macOS 使用 `Cmd`，不得显示 `CommandOrControl`）；托盘弹窗宽度由汉字标签、快捷键文本、间距和少量留白自然决定，不使用无关固定宽度，且不得先显示空白弹窗
- 截图窗口主色仅用于选区描边与尺寸标签
- 所有图标按钮大小接近汉字大小
...
- 备份导出的 zip 必须可重新导入，恢复后配置和历史记录应一致
...
补全全部翻译 / OCR 服务、TTS 服务、语言检测引擎、
服务拖拽排序、服务实例管理（添加/编辑/删除/启用禁用）。

### P4: 高级功能

HTTP API 服务器、备份与恢复（本地 zip；WebDAV 配置项保留但远端同步未实现）、自动更新、完整 i18n、
