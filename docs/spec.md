...
- **字典词典** — 选中单词查询词典释义（英文走英文词典、中文走中文词典）
- **文字识别** — 区域截图 → 文字识别
- **截图翻译** — 截图 → 识别 → 自动翻译
...
| 翻译 | `translate` | 宽度记忆值或默认宽度；高度自适应内容 | 语言转换区自然宽度 + 少量留白 / 内容自然最小高度 | 失焦可关闭、可固定、可置顶、frameless；宽度只能由用户调整并按配置记住，不允许内容、欢迎页或程序自动拉宽；高度随内容自适应 |
...
- **圆角**：窗口、卡片统一圆角 10px（固定值，不再暴露为用户可调项）
...
词典 / 文字识别 / 截图翻译窗口的顶部栏从左到右：置顶按钮、显示名 `Omni Pot`、当前模式标签。

设置窗口与更新器窗口**不显示**置顶 / 固定按钮，右上角使用普通的最小化 / 最大化 / 关闭三件套。

以上整体左对齐；其余窗口（翻译 / 词典 / 文字识别 / 截图翻译）右上角只保留关闭按钮。

按钮视觉规则：未激活时只显示主色描边图标，无背景框；激活时填充主色，内部线条反色（白色或深色按主题反转）。置顶按钮激活时自动联动开启固定。
...
字典词典窗口。选中文本后按字典词典快捷键弹出，按输入文字语言路由到单一词典：

- **英文输入** → 只渲染英文词典（`free_dictionary`），不显示任何中文词典
- **中文字 / 词 / 短语输入** → 只渲染中文词典（`chinese_dictionary` / `ecdict` 的中文释义），不显示任何英文词典

### 6.1 顶部栏

通用左对齐顶部栏（见 [4.3](#43-通用顶部栏)），模式标签为 `字典词典`。
...
- 按输入语言只调用对应一类词典服务的 `translate()`；返回 `DictResult`（`type='dict'`）的服务渲染为词典卡片
...
文字识别窗口与截图翻译窗口共用一份代码，按模式（`recognize` / `translate`）切换底栏按钮集。两者三段式布局相同。
...
- **左边**：展示截图图片（卡片）
- **右边**：
  - 文字识别模式：识别文本卡片（可编辑、可滚动）
  - 截图翻译模式：识别文本卡片 + 翻译文本卡片，**两张卡片视觉一致**（同背景、同字色、同 mono 标签），仅左侧文字"识别 / 翻译"区分

### 8.3 第三排 — 操作区

所有图标按钮大小接近汉字；语言下拉用与翻译窗口一致的 pill 样式（带背景与边框，**不带 AUTO/ZH 等字母前缀**）。

**文字识别模式按钮顺序：**

左侧：`复制图片 → 选择识别引擎（系统 OCR 等）→ 重新识别 → 自动检测`

右侧：`翻译 → 去除换行 → 去除空格 → 复制识别文本 → 导出`

**截图翻译模式按钮顺序：**

左侧：`复制图片 → 选择识别引擎 → 重新识别 → 自动检测 → 转换符号 → 简体中文 → 重新翻译`

右侧：`去除换行 → 去除空格 → 复制识别文本 → 导出`

按钮说明：

- **重新识别 / 重新翻译**：文字 pill 按钮（带背景），不是纯图标
...
- **外观卡片**：主题（`app_theme`：跟随系统 / 浅色 / 深色）、文字（`app_font` / `app_fallback_font`，与字号 `app_font_size` 同行展示，左侧字体下拉、右侧字号下拉）、主色调（`app_primary_color`，5 个圆形选色按钮）、透明背景（`transparent`）、托盘点击行为（`tray_click_event`）
...
**四个**全局快捷键录入框，每个有绑定 / 解绑按钮（未绑定显示"绑定"，已绑定显示"解绑"，**不使用 × 清除**）：

- **翻译**（UI 上是单行，内部对应 `hotkey_selection_translate` 与 `hotkey_input_translate` 两个 config 键，二者绑定到同一组合键）— 描述："有选中文本时翻译选中文本，否则弹出空输入窗口；剪贴板监听开启时自动翻译剪贴板新文本"
- **字典词典**（`hotkey_selection_dictionary`）
- **文字识别**（`hotkey_ocr_recognize`）
- **截图翻译**（`hotkey_ocr_translate`）

录入行为：捕获组合键、Backspace 清除、绑定按钮在主进程注册、成功 / 失败提示；已被其他应用占用时在"状态细节"区域以红色提示，**不在快捷键设置页常驻展示**。Wayland 限制属于故障排查或文档说明，不放快捷键页。

### 9.7 设置页: 服务

Tabs 切换**五类服务**：翻译 / 字典词典 / 文字识别 / 语音朗读 / 收藏。
...
| 配置键 | 操作 |
|---|---|
| `hotkey_selection_translate` / `hotkey_input_translate` | 翻译（UI 上合并为一项；两键建议绑同一组合键；有选中文本时翻译选中文本，否则弹出空输入窗口；剪贴板监听开启时自动翻译剪贴板新文本；窗口可见时切换隐藏） |
| `hotkey_selection_dictionary` | 字典词典（窗口可见时切换隐藏） |
| `hotkey_ocr_recognize` | 文字识别 |
| `hotkey_ocr_translate` | 截图翻译 |

划词翻译 / 字典词典：**必须在 main 进程、翻译窗口聚焦/创建之前**读取选区
（否则焦点切走会读错目标）。
...
- **空状态**：翻译窗口源文本为空且未被用户跳过时显示欢迎引导：标题"欢迎使用 Omni Pot"、副标题、**4 个快捷键提示卡（翻译 / 字典词典 / 文字识别 / 截图翻译，分别读取 `hotkey_selection_translate`（翻译显示该键，与 `hotkey_input_translate` 视为同键）/ `hotkey_selection_dictionary` / `hotkey_ocr_recognize` / `hotkey_ocr_translate`；未设置则显示"未设置"占位）**、"设置快捷键"按钮（打开设置窗口并跳到快捷键页）、"跳过"按钮（仅在本会话内隐藏，下次启动仍显示）。源文本一旦有内容欢迎区即消失，清空后若未跳过会再次出现。
...
- 用户面向功能统一为 4 项：翻译 / 字典词典 / 文字识别 / 截图翻译
...
- 欢迎页宽度匹配当前翻译窗口，不拉宽窗口；欢迎页高度刚好贴合内容
...
- 文字识别 / 截图翻译窗口第二排为左图右文布局；截图翻译模式下识别卡与翻译卡视觉一致
- 文字识别按钮顺序（左→右）：`复制图片 → 选择识别引擎 → 重新识别 → 自动检测 | 翻译 → 去除换行 → 去除空格 → 复制 → 导出`
- 截图翻译按钮顺序（左→右）：`复制图片 → 选择识别引擎 → 重新识别 → 自动检测 → 转换符号 → 简体中文 → 重新翻译 | 去除换行 → 去除空格 → 复制 → 导出`
- 文字识别 / 截图翻译窗口去掉图片尺寸、类型、识别字数、耗时等弱价值信息
- 字典词典窗口没有搜索框、没有换行按钮，支持单词级收藏；按输入语言只显示对应一类词典（英文→英文词典、中文→中文词典），不再显示词形变化卡片与来源 chips
- 设置窗口为左导航 + 右卡片布局，覆盖 8 个页面；侧边栏无置顶 / 固定按钮，标题栏使用最小化 / 最大化 / 关闭三件套
- 更新器窗口标题栏无置顶 / 固定按钮，只保留关闭按钮
- 快捷键设置只有 4 项（翻译 / 字典词典 / 文字识别 / 截图翻译），已绑定显示"解绑"按钮，未绑定显示"绑定"按钮；快捷键被占用提示在状态细节区域显示，不在快捷键页常驻
- 服务设置 5 个 Tab：翻译 / 字典词典 / 文字识别 / 语音朗读 / 收藏；编辑、删除使用带文字的完整按钮；无插件相关入口
- 设置 → 通用：界面语言归"应用"卡片；外观卡片含主题、文字（字体 + 字号）、主色调、透明背景、托盘点击行为
- 系统托盘菜单中文化为 4 个功能项 + 设置 / 更新 / 日志 / 重启 / 退出，每个功能项右侧显示完整快捷键（`Ctrl + Alt + ...`）；托盘弹窗宽度由汉字标签、快捷键文本、间距和少量留白自然决定，不使用无关固定宽度，且不得先显示空白弹窗
- 截图窗口主色仅用于选区描边与尺寸标签
- 所有图标按钮大小接近汉字大小
- 主题支持日 / 夜 × 5 主色共 10 种组合，颜色使用克制
- 圆角与布局密度固定，不再暴露为用户可调项；Tweaks 仅保留主题与主色
- 翻译窗口通用顶部栏左对齐顺序：**置顶 → 固定 → 软件名 → 模式标签**；右上角只保留关闭按钮
- 其他窗口（词典 / 文字识别 / 截图翻译）顶部栏：置顶 → 软件名 → 模式标签
...
剪贴板监听、翻译历史记录（SQLite）、字典词典 + CC-CEDICT 离线词典、设置页完整实现。
