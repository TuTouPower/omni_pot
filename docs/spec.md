...
- `docs/issues.md` — 当前实机验收发现的问题与必须回写到规格的产品要求
...
- **字典词典**（`hotkey_selection_dictionary`），不叫“查询字典”
- **文字识别**（`hotkey_ocr_recognize`）
- **截图翻译**（`hotkey_ocr_translate`）

录入行为：捕获组合键、Backspace 清除、绑定按钮在主进程注册、成功 / 失败提示；已被其他应用占用时在"状态细节"区域以红色提示，**不在快捷键设置页常驻展示**。快捷键展示必须按平台把 `CommandOrControl` 解析成用户可读修饰键：Windows / Linux 显示 `Control`，macOS 显示 `Command`。Wayland 限制属于故障排查或文档说明，不放快捷键页。

### 9.7 设置页: 服务

Tabs 切换**五类服务**：翻译 / 字典词典 / 文字识别 / 语音朗读 / 收藏。页面整体布局、服务卡片、字段展示和交互必须按 `docs/design/omni-pot/` 设计稿对齐。
...
- 快捷键设置只有 4 项（翻译 / 字典词典 / 文字识别 / 截图翻译），翻译是单一入口，不拆“划词翻译 / 输入翻译”；已绑定显示"解绑"按钮，未绑定显示"绑定"按钮；快捷键被占用提示在状态细节区域显示，不在快捷键页常驻
- 服务设置 5 个 Tab：翻译 / 字典词典 / 文字识别 / 语音朗读 / 收藏；编辑、删除使用带文字的完整按钮；无插件相关入口
- 设置 → 通用：界面语言归"应用"卡片；外观卡片含主题、文字（字体 + 字号）、主色调、透明背景、托盘点击行为；不提供代理功能或代理设置
