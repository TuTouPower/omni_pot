...
源词卡片下方紧接一张**发音 + 词性卡片**，展示第一个有结果的服务的 `pronunciations` 和 POS 标签（chip 样式，去重：同一种词性只显示一次）。无发音/词性数据时不渲染。
...
- 词典窗口顶部为可编辑源词卡片（contentEditable div，Enter 重查），下方为发音+词性卡片，再下方为各服务结果卡片；按检测语言路由到对应词典列表（英文→英文词典列表、中文→中文词典列表）；不再显示词形变化卡片与来源 chips
