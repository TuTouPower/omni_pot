...
- **字典词典** — 选中字词查询字典词典（英文走英文词典、中文走中文词典）
- **文字识别** — 区域截图 → 文字识别
- **截图翻译** — 截图 → 识别 → 自动翻译
...
通用左对齐顶部栏（见 [4.3](#43-通用顶部栏)），模式标签为 `词典`。

### 6.2 内容

- 每个词典服务返回 `DictResult`（`type='dict'`）时渲染一张卡片，卡片结构与翻译结果卡片一致（拖拽手柄 + 服务图标 + 名称 + 折叠箭头）
- 卡片内分三段：**发音**、**释义**（按 `partOfSpeech` 分组的编号义项）、**例句**（左侧细线 + 源/译双行）
...
每个发音独占一行：`region` 标签 + 音标文本（mono 字体）+ 朗读按钮。英文词典（如 Cambridge）同时返回 US/UK 发音时，两个发音合并到同一行（flex-wrap），每个音标后跟各自的朗读按钮。中文词典只有一个发音，单行显示。朗读按钮由 TTS 服务驱动（字典数据本身无音频字段）。

**顶部源词卡片**：

可编辑的查询词 + 语言检测 + 朗读 / 复制 / 收藏 / 查询按钮。
...
- 词典窗口没有搜索框、没有换行按钮，支持单词级收藏；按输入语言路由到对应词典列表（英文→英文词典列表、中文→中文词典列表），每个词典服务返回一张 DictResult 卡片；不再显示词形变化卡片与来源 chips
