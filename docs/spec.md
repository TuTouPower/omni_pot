...
Omni Pot logo + 版本号、简介、官网 / 文档 / 反馈 / 检查更新链接、诊断信息（日志目录 / 设置目录 / 本机 API 地址）。日志目录和设置目录行各带一个复制路径按钮；设置目录显示实际 `userData` 路径而非静态文字。诊断卡片内以 `ConfigRow` 行展示"导出日志"操作，说明文案："最近 7 天的日志打包为 zip，可附在反馈中"。
...
- 系统托盘菜单中文化为 4 个功能项 + 设置 / 更新 / 日志 / 重启 / 退出，每个功能项右侧显示快捷键（Windows / Linux 使用 `Ctrl`，macOS 使用 `Cmd`，不得显示 `CommandOrControl`）；托盘弹窗宽度由汉字标签、快捷键文本、间距和少量留白自然决定，不使用无关固定宽度，且不得先显示空白弹窗
- 截图窗口主色仅用于选区描边与尺寸标签
- 所有图标按钮大小接近汉字大小
