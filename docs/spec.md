...
Tabs 切换四类服务：翻译 / 识别 / TTS / 收藏。

- 每类显示已启用的**服务实例**列表，可拖拽排序（顶部优先）
- 每个实例项：拖拽手柄、服务图标、实例名、实例 key、标签、启用开关、编辑按钮、删除按钮
- 底部："添加内置服务" 按钮（打开内置服务选择）
- 服务实例配置表单为**先测试后保存**：保存时提交测试请求，成功才持久化
...
| 版本信息 | 当前版本 → 最新版本、发布日期、包大小 |
| 更新日志 | 渲染 release notes（Markdown） |
| 进度条 | 下载时显示，主色填充 |
| 操作按钮 | "稍后提醒"、"立即更新"（下载中禁用） |
