...
5. 注册全部 IPC handlers（config / window / hotkey / text / ocr / history / backup / chineseDict）
...
| `text` | `getSelection` / `writeClipboard` / `writeClipboardImage` / `onTranslateSelectionPending` / `onTranslateFromSelection` / `onInputTranslate` / `onTranslateFromApi` / `onTranslateFromClipboard` / `onDictLookup` / `onTranslateSelectionEmpty` / `onDictSelectionEmpty` |
| `ocr` | `captureScreenshot` / `openRecognize` / `sendToTranslate` / `systemRecognize` / `onScreenshotShow` / `onRecognizeShow` |
| `history` | `add` / `list` / `count` / `update` / `delete` / `clear` / `serviceKeys` |
