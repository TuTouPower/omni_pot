...
源词卡片下方紧接一张**发音卡片**，展示第一个有结果的服务的 `pronunciations`。无发音数据时不渲染。释义区域按词性分组，每个词性作为小标题（chip 样式），下方为该词性对应的编号义项（从 01 重新编号）。
...
| `dictionary_service_list` | string[] | 见下方说明 | 词典实例顺序；中文词典列表默认 `['chinese_dictionary@default']`，英文词典列表默认 `['cambridge_dict@default']` |
...
- 系统托盘菜单中文化为 4 个功能项 + 设置 / 支持作者（紫色字色） / 更新 / 日志 / 重启 / 退出，每个功能项右侧显示完整快捷键（Windows / Linux 使用 `Control`，macOS 使用 `Command`，不得显示 `CommandOrControl`）；托盘弹窗宽度由汉字标签、快捷键文本、间距和少量留白自然决定，不使用无关固定宽度，且不得先显示空白弹窗
- 截图窗口主色仅用于选区描边与尺寸标签
- 所有图标按钮大小接近汉字大小
