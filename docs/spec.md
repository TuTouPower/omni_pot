...
- **英文输入** → 调用英文词典列表（默认 `cambridge_dict`，`free_dictionary` 默认关闭）
- **中文输入** → 调用中文词典列表（默认 `chinese_dictionary`）
...
`free_dictionary@default`、`chinese_dictionary@default`、`cambridge_dict@default`、`system_tts@default`、
...
| 21 | CC-CEDICT | `ecdict` | 免费，中英词典，输出词典结果 |
...
| `dictionary_service_list` | string[] | `['chinese_dictionary@default', 'ecdict@default']` | 中文词典实例顺序 |
