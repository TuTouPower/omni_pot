...
| Vitest 单测 + Playwright e2e | Git hooks / CI 门禁 |
| Knip 死代码 / 依赖检查 | |
| Biome scoped format check | |
...
- `security` 先纳入 `npm audit --audit-level=high`;Gitleaks / osv-scanner / Semgrep 放到第三档继续补。
...
- 第二档:`npm run deadcode` 无未用项;`npm run security` 无 high/critical 漏洞;`npm run format:check` 对当前纳入范围无 diff。
