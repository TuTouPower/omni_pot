...
const BAIDU_FIELD_LANG_MAP: Record<string, string> = {
    auto: 'auto',
    zh_cn: 'zh',
    zh_tw: 'cht',
    yue: 'yue',
...
export const baiduFieldService: TranslateService = {
    key: 'baidu_field',
    name: 'Baidu Field',
    languages: BAIDU_FIELD_LANGUAGES,
    ...
    async translate(
        ...
        const field = (config.field as string) || 'it'
        const salt = Math.random().toString(36).substring(2)
        const sign = md5(`${appid}${text}${salt}${secret}`)

        const fromLang = BAIDU_FIELD_LANG_MAP[from] ?? from
        const toLang = BAIDU_FIELD_LANG_MAP[to] ?? to
