...
function has_entries(entries: DictEntry[]): entries is [DictEntry, ...DictEntry[]] {
...
export const freeDictionaryService: TranslateService = {
    ...
    async translate(
        ...
        if (!resp.ok) {
            throw new Error(`Free Dictionary API ${String(resp.status)}`)
        }

        const entries = await resp.json() as DictEntry[]
        if (!Array.isArray(entries) || !has_entries(entries)) {
