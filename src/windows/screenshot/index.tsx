...
import { try_qr_decode } from '../qr_decode'
...
export default function ScreenshotWindow(): React.ReactElement {
    ...
    const confirm_selection = useCallback(async () => {
        ...
        try {
            ...
            if (mode === 'translate') {
                await window.electronAPI.ocr.open_recognize(cropped, full_text, 'translate')
            } else {
                await window.electronAPI.ocr.open_recognize(cropped, full_text, 'recognize')
            }
        } catch (err) {
